...
class FrameAnnotator:
    ...
    def annotate_tracking_areas(self, tracking_areas: Iterable[TrackingArea]) -> Self:
        ...
        font_scale = int(0.0015 * self._xy_dimensions[0])
        font_thickness = int(0.0026 * self._xy_dimensions[0])
        ...
        for area, color in zip(tracking_areas, colors):
            ...
            text = f"tag: {area.tag}"
            text_box_width, text_box_height = cv2.getTextSize(
                text, cv2.FONT_HERSHEY_SIMPLEX, font_scale, font_thickness
            )[0]
            # TODO: Place text at left bottom of polygon
            p1 = polygon[0]
            p2 = (polygon[0][0] + text_box_width, polygon[0][1] - text_box_height)
